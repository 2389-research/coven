// ABOUTME: Error types for the fold-pack SDK.
...
impl From<fold_ssh::SshError> for PackError {
    fn from(err: fold_ssh::SshError) -> Self {
        PackError::AuthenticationFailed(err.to_string())
    }
}

impl From<fold_grpc::GrpcClientError> for PackError {
    fn from(err: fold_grpc::GrpcClientError) -> Self {
        match err {
            fold_grpc::GrpcClientError::ConnectionFailed(msg) => {
                PackError::ConnectionFailed(msg)
            }
            fold_grpc::GrpcClientError::StreamClosed => PackError::StreamClosed,
            fold_grpc::GrpcClientError::StreamError(msg) => PackError::StreamError(msg),
            fold_grpc::GrpcClientError::AuthenticationFailed(msg) => {
...
mod tests {
    ...
    fn test_from_grpc_error() {
        let grpc_err = fold_grpc::GrpcClientError::StreamClosed;
        let err: PackError = grpc_err.into();
        assert!(matches!(err, PackError::StreamClosed));

        let grpc_err = fold_grpc::GrpcClientError::ConnectionFailed("refused".to_string());
        let err: PackError = grpc_err.into();
    ...
    fn test_from_grpc_error_stream_error() {
        let grpc_err = fold_grpc::GrpcClientError::StreamError("broken pipe".to_string());
        let err: PackError = grpc_err.into();
        assert!(matches!(err, PackError::StreamError(_)));
    ...
    fn test_from_grpc_error_authentication_failed() {
        let grpc_err =
            fold_grpc::GrpcClientError::AuthenticationFailed("invalid key".to_string());
        let err: PackError = grpc_err.into();
    ...
    fn test_from_grpc_error_other() {
        let grpc_err = fold_grpc::GrpcClientError::RegistrationRejected {
    ...
    fn test_from_ssh_error() {
        let ssh_err = fold_ssh::SshError::UnsupportedKeyType("rsa".to_string());
