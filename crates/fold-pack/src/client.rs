// ABOUTME: PackClient for connecting to fold-gateway and serving tools.
...
use fold_proto::pack_service_client::PackServiceClient;
use fold_proto::{ExecuteToolResponse, PackManifest};
use fold_ssh::{load_key, PrivateKey, SshAuthCredentials};
use std::path::Path;
use std::sync::Arc;
...
/// Client for connecting a tool pack to fold-gateway.
///
...
///
/// ```ignore
/// use fold_pack::{PackClient, ManifestBuilder, ToolHandler, ToolError};
...
impl PackClient {
    /// Connect to a fold-gateway server.
    ///
    /// # Arguments
    ///
                    ...
                    let response = match result {
                        Ok(ref output) => {
                            ...
                            ExecuteToolResponse {
                                request_id,
                                result: Some(
                                    fold_proto::execute_tool_response::Result::OutputJson(output.clone()),
                                ),
                            }
                        }
                        Err(e) => {
                            ...
                            ExecuteToolResponse {
                                request_id,
                                result: Some(fold_proto::execute_tool_response::Result::Error(
                                    format_tool_error(&e),
                                )),
                            }
                        }
