...
impl Config {
    ...
    pub fn load(gateway_override: Option<&str>, theme_override: Option<&str>) -> Result<Self> {
        ...
        if let Ok(host) = std::env::var("FOLD_GATEWAY_HOST") {
            config.gateway.host = host;
        }
        if let Ok(port) = std::env::var("FOLD_GATEWAY_PORT") {
            if let Ok(p) = port.parse::<u16>() {
                config.gateway.port = p;
            }
        }
        // Support FOLD_GATEWAY_TLS env var
        if let Ok(tls) = std::env::var("FOLD_GATEWAY_TLS") {
            config.gateway.use_tls = tls == "1" || tls.eq_ignore_ascii_case("true");
        }
        // Support legacy FOLD_GATEWAY_URL for backwards compatibility
        if let Ok(url) = std::env::var("FOLD_GATEWAY_URL") {
        ...
        if let Ok(theme) = std::env::var("FOLD_THEME") {
    ...
    pub fn config_path() -> Result<PathBuf> {
        ...
        Ok(config_dir.join("fold").join("tui-config.toml"))
    }

    pub fn state_path() -> Result<PathBuf> {
        let config_dir = dirs::config_dir()
            .ok_or_else(|| AppError::Config("Could not find config directory".to_string()))?;
        Ok(config_dir.join("fold").join("tui-state.json"))
    }
}
