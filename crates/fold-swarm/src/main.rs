// ABOUTME: fold-swarm CLI entry point.
...
#[derive(Parser)]
#[command(name = "fold-swarm")]
#[command(about = "Supervisor for fold workspace agents")]
struct Cli {
    #[command(subcommand)]
    command: Commands,
}
...
#[derive(Subcommand)]
enum Commands {
    /// Initialize a new fold-swarm configuration
    Init,
...
async fn main() -> anyhow::Result<()> {
    ...
    match cli.command {
        ...
        Commands::Agent {
            ...
            use fold_swarm_core::Config;
            use fold_core::backend::{MuxBackend, MuxConfig};
            use fold_swarm_backend::BackendHandle;
            use fold_swarm_backend::dispatch_tools::{ListAgentsTool, CreateWorkspaceTool, DeleteWorkspaceTool};
            use crate::agent::{GatewayClient, Session};
            use std::sync::Arc;
            use tokio::sync::Mutex;

            // Load or generate SSH key
            let key_path = fold_ssh::default_swarm_key_path()
                .ok_or_else(|| anyhow::anyhow!("Could not determine SSH key path"))?;
            let private_key = fold_ssh::load_or_generate_key(&key_path)?;
            let fingerprint = fold_ssh::compute_fingerprint(private_key.public_key())?;
            ...
            let (handle, backend_name) = if dispatch_mode {
                // Dispatch mode uses fold-core's MuxBackend with dispatch tools
                ...
                #[cfg(feature = "acp")]
                {
                    use fold_swarm_backend::acp::{AcpBackend, AcpConfig};
                ...
                #[cfg(not(feature = "acp"))]
                {
                    // Fallback to fold-core's MuxBackend when ACP is not available
                    let mux_config = MuxConfig {
                        model: "claude-sonnet-4-20250514".to_string(),
                        max_tokens: 8192,
                        working_dir: working_dir.clone(),
