// ABOUTME: Configuration loading for coven-pack SDK with file, env, and default precedence.
// ABOUTME: Resolves gateway URL from env vars, .env files, and ~/.config/coven/packs.toml.
...
/// Raw TOML structure for ~/.config/coven/packs.toml
...
pub struct PackConfig {
    ...
    /// Path to SSH key: ~/.config/coven/packs/{pack_name}/id_ed25519
...
impl PackConfig {
    ...
    /// 1. `COVEN_GATEWAY_URL` (full URL, e.g., "https://gw.example.com:50051")
    /// 2. `GATEWAY_ADDR` (legacy compat, treated as full URL)
    /// 3. Constructed from `COVEN_SERVER`/`COVEN_PORT` + packs.toml + defaults
    ///
    /// SSH key path resolution order:
    /// 1. `COVEN_SSH_KEY_PATH` (explicit path)
    /// 2. `PACK_SSH_KEY` (legacy compat)
    /// 3. Default: ~/.config/coven/packs/{pack_name}/id_ed25519
    pub fn load(pack_name: &str) -> Result<Self, PackError> {
        ...
        let gateway_url = std::env::var("COVEN_GATEWAY_URL")
            .ok()
            .filter(|s| !s.is_empty())
            ...
            .unwrap_or_else(|| {
                ...
                let server = std::env::var("COVEN_SERVER")
                    .ok()
                    .filter(|s| !s.is_empty())
                    .or(toml_config.server)
                    .unwrap_or_else(|| "localhost".to_string());

                let port = std::env::var("COVEN_PORT")
                    .ok()
        ...
        let ssh_key_path = std::env::var("COVEN_SSH_KEY_PATH")
            .ok()
            .filter(|s| !s.is_empty())
...
/// Load ~/.config/coven/packs.toml, returning defaults if file doesn't exist or can't be parsed.
...
/// Get default SSH key path for a pack: ~/.config/coven/packs/{pack_name}/id_ed25519
...
mod tests {
    ...
    fn test_load_packs_toml_returns_defaults_when_file_missing() {
        ...
        // On most CI/test systems ~/.config/coven/packs.toml won't exist.
    ...
    fn test_default_pack_key_path_constructs_expected_path() {
        ...
        assert!(path.ends_with("coven/packs/my-pack/id_ed25519"));

        // Should contain the config directory prefix
        let path_str = path.to_string_lossy();
        assert!(
            path_str.contains("coven/packs/my-pack/id_ed25519"),
            "Path was: {}",
            path_str
        );
    }
    ...
    fn test_default_pack_key_path_different_pack_names() {
        ...
        assert!(path_a.ends_with("coven/packs/alpha-pack/id_ed25519"));
        assert!(path_b.ends_with("coven/packs/beta-pack/id_ed25519"));
        assert_ne!(path_a, path_b);
    }

    #[test]
    fn test_config_dir_returns_coven_subdir() {
        let dir = config_dir();
        assert!(dir.is_some());
        let dir = dir.unwrap();
        assert!(
    ...
    fn test_pack_config_load_with_defaults() {
        ...
        without_env_vars(&["COVEN_SERVER", "COVEN_PORT"], || {
            let config = PackConfig::load("test-pack").unwrap();
            assert_eq!(config.gateway_url, "http://localhost:50051");
            assert!(config
                .ssh_key_path
                .ends_with("coven/packs/test-pack/id_ed25519"));
        });
    }

    #[test]
    fn test_pack_config_load_env_server_override() {
        with_env_vars(&[("COVEN_SERVER", "custom.example.com")], || {
            // Remove COVEN_PORT to test server-only override
    ...
    fn test_pack_config_load_both_env_overrides() {
        with_env_vars(
            &[("COVEN_SERVER", "prod.example.com"), ("COVEN_PORT", "8080")],
            || {
                let config = PackConfig::load("test-pack").unwrap();
    ...
    fn test_pack_config_load_invalid_port_uses_default() {
        with_env_vars(&[("COVEN_PORT", "not-a-number")], || {
            std::env::remove_var("COVEN_SERVER");
            let config = PackConfig::load("test-pack").unwrap();
    ...
    fn test_pack_config_ssh_key_path_varies_by_pack_name() {
        without_env_vars(&["COVEN_SERVER", "COVEN_PORT"], || {
            ...
            assert!(config_a
                .ssh_key_path
                .ends_with("coven/packs/pack-alpha/id_ed25519"));
            assert!(config_b
                .ssh_key_path
                .ends_with("coven/packs/pack-beta/id_ed25519"));
    ...
    fn test_pack_config_load_legacy_gateway_addr() {
        without_env_vars(&["COVEN_GATEWAY_URL", "COVEN_SERVER", "COVEN_PORT"], || {
    ...
    fn test_pack_config_gateway_url_takes_priority_over_legacy() {
        with_env_vars(
            &[
                ("COVEN_GATEWAY_URL", "https://new-gw:443"),
    ...
    fn test_pack_config_ssh_key_path_env_override() {
        with_env_vars(&[("COVEN_SSH_KEY_PATH", "/custom/key/path")], || {
    ...
    fn test_pack_config_legacy_ssh_key_env() {
        without_env_vars(&["COVEN_SSH_KEY_PATH"], || {
