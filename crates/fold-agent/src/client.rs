// ABOUTME: GRPC client implementation for fold-agent
// ABOUTME: Handles connection, registration, message processing loop

use anyhow::{Result, bail};
use fold_core::backend::{
...
use fold_proto::fold_control_client::FoldControlClient;
use fold_proto::{AgentMessage, MessageResponse, RegisterAgent, agent_message, server_message};
use fold_ssh::{
...
pub async fn run(
    ...
    let fold = Fold::new(&config, backend).await?;
    ...
    eprintln!("  (Register this fingerprint with the gateway using fold-admin)");
    ...
    let mut client = FoldControlClient::with_interceptor(channel, ssh_auth_interceptor);
    ...
    // Wrap fold in Arc for sharing with spawned tasks
        ...
        match msg.payload {
            ...
            Some(server_message::Payload::SendMessage(send_msg)) => {
                ...
                tokio::spawn(async move {
                    ...
                    process_message(fold_clone, incoming, request_id, tx_clone, verbose).await;
                    eprintln!("Ready and waiting for messages...");
            ...
            Some(server_message::Payload::PackToolResult(result)) => {
                let status = match &result.result {
                    Some(fold_proto::pack_tool_result::Result::OutputJson(_)) => "✓ success",
                    Some(fold_proto::pack_tool_result::Result::Error(_)) => "✗ error",
                    None => "? empty",
                };
...
async fn process_message(
    ...
    match fold.handle(incoming).await {
        Ok(mut stream) => {
        ...
        Err(e) => {
            ...
            let error_response = AgentMessage {
                payload: Some(agent_message::Payload::Response(MessageResponse {
                    ...
                    event: Some(fold_proto::message_response::Event::Error(e.to_string())),
                })),
            };
            ...
            let done_response = AgentMessage {
                payload: Some(agent_message::Payload::Response(MessageResponse {
                    request_id: request_id.clone(),
                    event: Some(fold_proto::message_response::Event::Done(
                        fold_proto::Done {
...
async fn convert_event_to_response(request_id: &str, event: OutgoingEvent) -> AgentMessage {
    use fold_proto::message_response::Event;

    let event = match event {
        OutgoingEvent::Thinking => Event::Thinking("thinking...".to_string()),
        OutgoingEvent::Text(s) => Event::Text(s),
        OutgoingEvent::ToolUse { id, name, input } => Event::ToolUse(fold_proto::ToolUse {
            id,
            name,
        ...
        OutgoingEvent::ToolResult {
            ...
            output,
            is_error,
        } => Event::ToolResult(fold_proto::ToolResult {
            id,
            output,
            is_error,
        }),
        OutgoingEvent::Done { full_response } => Event::Done(fold_proto::Done { full_response }),
        OutgoingEvent::Error(e) => Event::Error(e),
        OutgoingEvent::ToolApprovalRequest { id, name, input } => {
            Event::ToolApprovalRequest(fold_proto::ToolApprovalRequest {
                id,
                name,
                input_json: input.to_string(),
            })
                ...
                Ok(metadata) => {
                    ...
                    if size > MAX_FILE_SIZE_BYTES {
                        ...
                        match tokio::fs::read(&path).await {
                            Ok(data) => Event::File(fold_proto::FileData {
                                filename,
                                mime_type,
                                data,
                            }),
        ...
        OutgoingEvent::SessionInit { session_id } => {
            Event::SessionInit(fold_proto::SessionInit { session_id })
        }
        OutgoingEvent::SessionOrphaned => Event::SessionOrphaned(fold_proto::SessionOrphaned {
        ...
        OutgoingEvent::Usage {
            ...
            cache_write_tokens,
            thinking_tokens,
        } => Event::Usage(fold_proto::TokenUsage {
            input_tokens,
            output_tokens,
            cache_read_tokens,
            cache_write_tokens,
        ...
        OutgoingEvent::ToolState { id, state, detail } => {
            Event::ToolState(fold_proto::ToolStateUpdate {
                id,
...
/// Convert OutgoingEvent tool state string to fold_proto ToolState enum value
fn outgoing_tool_state_to_proto(state: &str) -> i32 {
    match state {
        "pending" => fold_proto::ToolState::Pending as i32,
        "awaiting_approval" => fold_proto::ToolState::AwaitingApproval as i32,
        "running" => fold_proto::ToolState::Running as i32,
        "completed" => fold_proto::ToolState::Completed as i32,
        "failed" => fold_proto::ToolState::Failed as i32,
        "denied" => fold_proto::ToolState::Denied as i32,
        "timeout" => fold_proto::ToolState::Timeout as i32,
        "cancelled" => fold_proto::ToolState::Cancelled as i32,
        _ => fold_proto::ToolState::Unspecified as i32,
    }
}
