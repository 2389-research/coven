// ABOUTME: fold-agent binary - connects to control server via GRPC
...
pub use fold_agent::build_mcp_url;
use std::path::PathBuf;

#[derive(Parser)]
#[command(name = "fold-agent")]
...
struct Cli {
    ...
    #[arg(short, long, env = "FOLD_BACKEND", global = true)]
    ...
    /// Load configuration from a file (default: ~/.config/fold/agent.toml)
    #[arg(long, global = true)]
...
/// Get default config path (~/.config/fold/agent.toml)
fn default_config_path() -> Option<PathBuf> {
    xdg_config_dir().map(|p| p.join("agent.toml"))
}

/// Resolve project name from .fold/project.toml or directory basename
fn resolve_project_name(working_dir: &std::path::Path) -> String {
    // Try to load .fold/project.toml (no exists() check - read_to_string handles it)
...
async fn run_agent(
    ...
    let config_path = config.or_else(|| {
        // Check for project-local config first (.fold/agent.toml in cwd)
        ...
        // Fall back to user-global config (~/.config/fold/agent.toml)
        ...
        config_path
    {
        ...
        let config = if let Some(agent_ref) = config.get("agent").and_then(|v| v.as_str()) {
            // Resolve to ~/.config/fold/agents/{agent}.toml
        ...
        bail!(
            "No configuration found. Create one with 'fold-agent new' or specify --config.\n\
             Searched:\n\
             - .fold/agent.toml (project-local)\n\
             - ~/.config/fold/agent.toml (user-global)"
        );
    } else {
    ...
    // Project name comes from: .fold/project.toml > directory basename
