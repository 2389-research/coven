// ABOUTME: Configuration loading and management for fold
...
impl Config {
    /// Get the XDG config directory for fold (~/.config/fold)
    ...
    /// Get the XDG data directory for fold (~/.local/share/fold)
...
# Location: ~/.config/fold/config.toml

# Workspace directory to launch from
workspace = "{home}"

[database]
# path = "~/.local/share/fold/threads.db"  # Default location
