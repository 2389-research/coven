...
fn generate_idempotency_key() -> String {
    ...
    let timestamp = std::time::SystemTime::now()
        ...
        .unwrap_or_default()
        .as_millis();
    format!("coven-client-{}-{}", timestamp, count)
}
...
impl CovenClient {
    ...
    pub fn new_with_auth(gateway_url: String, ssh_key_path: &Path) -> Result<Self, CovenError> {
        Self::warn_if_insecure_with_auth(&gateway_url);
        let key = load_or_generate_key(ssh_key_path)
            .map_err(|e| CovenError::Connection(format!("failed to load SSH key: {}", e)))?;
    ...
    ///
    /// Local interceptor errors are marked with `x-coven-local-error` metadata
    ...
    fn local_interceptor_error(message: String) -> tonic::Status {
        ...
        status
            .metadata_mut()
            .insert("x-coven-local-error", "true".parse().unwrap());
        status
    }
    ...
    fn runtime(&self) -> &tokio::runtime::Runtime {
        self.runtime
            .as_ref()
            .expect("CovenClient runtime was shut down")
    }
        ...
        match &result {
            ...
            Err(status) => {
                ...
                let is_local_interceptor_error = status.code() == tonic::Code::Internal
                    && status.metadata().get("x-coven-local-error").is_some();
                ...
                if is_local_interceptor_error {
                    ...
                    return Err(CovenError::Connection(format!(
                        "SSH auth failed: {}",
                        status.message()
                    )));
                }
    ...
    pub fn refresh_agents(&self) -> Result<Vec<Agent>, CovenError> {
        self.runtime().block_on(self.refresh_agents_async())
    }

    /// Async implementation of refresh_agents - use this from async contexts
    pub async fn refresh_agents_async(&self) -> Result<Vec<Agent>, CovenError> {
        let channel = self.create_channel_internal().await?;
    ...
    pub fn load_history(&self, agent_id: String) -> Result<Vec<Message>, CovenError> {
        self.runtime().block_on(self.load_history_async(agent_id))
    }

    /// Async implementation of load_history - use this from async contexts
    pub async fn load_history_async(&self, agent_id: String) -> Result<Vec<Message>, CovenError> {
    ...
    pub fn send_message(&self, agent_id: String, content: String) -> Result<(), CovenError> {
        let mut state_guard = self.state.write().expect("lock poisoned");
        ...
        let agent = state_guard
            ...
            .find(|a| a.id == agent_id)
            .cloned()
            .ok_or_else(|| CovenError::AgentNotFound(agent_id.clone()))?;
