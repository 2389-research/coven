...
impl App {
    pub fn new(config: &Config) -> Result<Self> {
        ...
        let client = Arc::new(
            CovenClient::new_with_auth(gateway_url.clone(), &key_path)
    ...
    async fn handle_app_event(&mut self, event: AppEvent) {
        match event {
                            ...
                            match client.send_message(agent_id_clone.clone(), content) {
                                ...
                                Err(e) => {
                                    ...
                                    let _ = tx.send(AppEvent::StreamEvent {
                                        ...
                                        event: coven_client::StreamEvent::Error {
            ...
            AppEvent::StreamEvent { agent_id, event } => {
                ...
                if !is_current_agent {
                    if let coven_client::StreamEvent::Done = &event {
                ...
                match event {
                    coven_client::StreamEvent::Text { content } => {
                    ...
                    coven_client::StreamEvent::Thinking { content } => {
                        if self.chat.active_stream_mut().is_none() {
                            self.chat.set_active_stream(ActiveStream::new());
                        }
                        if let Some(stream) = self.chat.active_stream_mut() {
                    ...
                    coven_client::StreamEvent::ToolUse { name, input } => {
                        if self.chat.active_stream_mut().is_none() {
                            self.chat.set_active_stream(ActiveStream::new());
                        }
                        if let Some(stream) = self.chat.active_stream_mut() {
                    ...
                    coven_client::StreamEvent::Done => {
                        self.chat.clear_active_stream();
                    }
                    coven_client::StreamEvent::Error { message } => {
                        tracing::error!("Stream error: {}", message);
                        self.chat.clear_active_stream();
                    }
                    coven_client::StreamEvent::Usage { info } => {
                        self.session_usage.accumulate(&info);
                    }
                    coven_client::StreamEvent::ToolState { state, detail } => {
                        if self.chat.active_stream_mut().is_none() {
                            self.chat.set_active_stream(ActiveStream::new());
                        }
                        if let Some(stream) = self.chat.active_stream_mut() {
            ...
            AppEvent::MessagesChanged { agent_id } => {
                ...
                if Some(&agent_id) == self.current_agent_id.as_ref() {
                    // Get live messages from the CovenClient cache (not local persistence)
...
struct ArcStreamCallback(Arc<dyn coven_client::StreamCallback>);

impl coven_client::StreamCallback for ArcStreamCallback {
    fn on_event(&self, agent_id: String, event: coven_client::StreamEvent) {
        self.0.on_event(agent_id, event);
    }
}

// Wrapper to adapt Arc<dyn StateCallback> to Box<dyn StateCallback>
struct ArcStateCallback(Arc<dyn coven_client::StateCallback>);

impl coven_client::StateCallback for ArcStateCallback {
