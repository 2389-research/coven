// ABOUTME: Public entry point for running the TUI from other crates.
// ABOUTME: Contains the event loop, terminal setup, and orchestration logic.

use anyhow::{Context, Result};
use coven_link::config::CovenConfig;

use crate::app::{Action, App};
use crate::client::{Client, Response, StateChange};
use crate::ui;
use crossterm::{
...
/// Run the TUI chat interface (synchronous entry point).
/// Creates its own tokio runtime. Use this from non-async contexts
/// (e.g., the standalone `coven-chat` binary).
pub fn run(initial_agent: Option<String>) -> Result<()> {
    // Initialize file-based logging for TUI (must happen before terminal setup)
    ...
    rt.block_on(run_app(initial_agent, client))
}

/// Run the TUI chat interface (async entry point).
/// Use this when already inside a tokio runtime (e.g., from coven-cli's `#[tokio::main]`).
/// Avoids creating a nested runtime, which would panic on `block_on`.
pub async fn run_async(initial_agent: Option<String>) -> Result<()> {
    // Initialize file-based logging for TUI (must happen before terminal setup)
    coven_log::init_file("tui");
    ...
    // Create client (CovenClient creates its own runtime internally, which is safe)
    let key_path = ssh_key_path()?;
    let client = Client::new(&gw_url, &key_path)?;

    run_app(initial_agent, client).await
}
